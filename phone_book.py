...
def yes_no(message_y_n = "Да/Нет (д/н | y/n): "):
...
def find_contact():
    find_name = input('Поиск по всем полям: ').upper()
    none_contact = True
    j = 0
    for i in phone_book_list:
        if find_name in i.upper():
            print('Контакт найден:', j, contact_for_print(i))
            none_contact = False
        j += 1
    if none_contact:
...
def del_find_contact():
    find_name = input('Поиск контакта для удаления: ').upper()
    none_contact = True
    j = 0
    for i in phone_book_list:
        if find_name in i.upper():
            ...
            none_contact = False
            print('Удалить контакт?', j, contact_for_print(i))
            if yes_no():
                phone_book_list.remove(i)
                global flag_of_change
                flag_of_change = True
...
def edit_find_contact():
    find_name = input('Поиск контакта для измененияния: ').upper()
    none_contact = True
    j = 0
    for i in phone_book_list:
        if find_name in i.upper():
            print('Найден контакт:', j, contact_for_print(i))
            none_contact = False
            old_contact = contact_for_print(i)
            print('Изменить?', j, old_contact)
            if yes_no():
                last_name, first_name, patronim, telefon = input_contact()
                print("контакт", old_contact)
                # i = last_name.upper() + ', ' +  first_name.upper() + ', ' +patronim.upper()+ ', '+  telefon
                phone_book_list[j] = last_name.title() + ', ' +  first_name.title() + ', ' +patronim.title()+ ', '+  telefon
